...
function init_getusermedia_simple() {
  // solution issue de https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia
      ...
      navigator.webkitGetUserMedia(constraints, function(mediaStream) {
        ...
        M.toast({html:"c'est censé marcher"});
        console.log("c'est censé marcher");
        var video = document.querySelector('video');

    try {    video.srcObject = mediaStream;
} catch (error) {
  video.src = window.URL.createObjectURL(mediaStream);
  console.log("pas de srcobject");
}
...
*/
          ...
          console.log("plop loadé");
          //video.play();
          //console.log(video.play());
        
          try {    video.play();
            console.log("plop play");
          } catch (error) {
...
/*
        ...
        M.toast({html:"là y a eu une erreur"});
